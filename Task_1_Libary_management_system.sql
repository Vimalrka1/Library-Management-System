...
# 1.1 a, Create 'Books' table with columns (BOOK_ID, TITLE, AUTHOR, GENRE, YEAR_PUBLISHED,AVAILABLE_COPIES)
Create table Books 
...
#1.1 B) Create Members table with columns (MEMBER_ID, NAME, EMAIL, PHONE_NO, ADDRESS, MEMBERSHIP_DATE)
...
#1.1 C) Create BorrowingRecords table with columns BORROW_ID, MEMBER_ID,BOOK_ID, BORROW_DATE, RETURN_DATE. 
...
# 1.2: Data Creation
# 1.2 a) Insert sample data into the table: Books
...
(171,'The Lord of the Rings','Tolkien','Fantasy',2001,12);
 ...
 #1.2 b) Insert sample data into the table : Members
...
(107,'Kamaraj','Kamaraj@gmail.com',9176220418,  'Medavakkam,Chennai','2025-05-31');

# 1.2 C) Insert sample data into the table: BorrowingRecords
...
# 1.3 A) Retrieve a list of books currently borrowed by a specific member.

select b.book_id,m.Name as Borrowed_by,bk.TITLE as list_of_books
from BorrowingRecords b
join books bk on b.BOOK_ID = bk.BOOK_ID
join Members m on b.MEMBER_ID = m.MEMBER_ID
order by b.book_id asc;

#1.3 B)Find members who have overdue books (borrowed more than 30 days ago, not returned)
...
#1.3 c)# Retrieve books by genre along with the count of available copies
...
#1.3 d) Find the most borrowed book(s) overall.
...
#1.3 e) Retrieve members who have borrowed books from at least three different genres.
...
#1.4 b) Find the top three most active members based on the number of books borrowed. 
...
#1.4 C, Retrieve authors whose books have been borrowed at least 10 times.
...
#1.4 d)#Identify members who have never borrowed a book.
